...
.brand-name { font-family: 'Playfair Display', serif; font-size: 28px; font-weight: 900; color: #ffffff; }
.brand-tag { font-family: 'IBM Plex Mono', monospace; font-size: 10px; color: #f0a500; border: 1px solid #c47e00; padding: 3px 8px; border-radius: 3px; margin-left: 10px; }
.masthead-stats { display: flex; gap: 32px; }
.mstat-val { font-family: 'IBM Plex Mono', monospace; font-size: 18px; font-weight: 600; color: #ffffff; }
.mstat-lbl { font-size: 10px; color: #6b6b7a; text-transform: uppercase; letter-spacing: 1.5px; }
.kpi { background: #16161a; border: 1px solid #2a2a35; border-radius: 12px; padding: 20px 22px; position: relative; overflow: hidden; }
.kpi::before { content: ''; position: absolute; top: 0; left: 0; width: 3px; height: 100%; background: #f0a500; }
.kpi-icon { font-size: 22px; margin-bottom: 8px; display: block; }
.kpi-num { font-family: 'Playfair Display', serif; font-size: 28px; font-weight: 700; color: #ffffff; }
.kpi-lbl { font-size: 10px; color: #6b6b7a; text-transform: uppercase; letter-spacing: 2px; margin-top: 4px; }
.kpi-delta { font-family: 'IBM Plex Mono', monospace; font-size: 11px; margin-top: 6px; }
.up { color: #34d399; }
.down { color: #f87171; }
.panel { background: #16161a; border: 1px solid #2a2a35; border-radius: 14px; padding: 22px; margin-bottom: 20px; }
.panel-title { font-family: 'IBM Plex Mono', monospace; font-size: 10px; color: #6b6b7a; text-transform: uppercase; letter-spacing: 2.5px; margin-bottom: 16px; padding-bottom: 12px; border-bottom: 1px solid #2a2a35; }
.sec-label { font-family: 'IBM Plex Mono', monospace; font-size: 10px; color: #f0a500; text-transform: uppercase; letter-spacing: 3px; margin-bottom: 12px; display: flex; align-items: center; gap: 10px; }
.sec-label::after { content: ''; flex: 1; height: 1px; background: #2a2a35; margin-left: 12px; }
.clf-good { background: linear-gradient(135deg, #052e1c, #065f46); border: 2px solid #34d399; border-radius: 14px; padding: 24px; text-align: center; }
.clf-bad { background: linear-gradient(135deg, #2d0a0a, #450a0a); border: 2px solid #f87171; border-radius: 14px; padding: 24px; text-align: center; }
.clf-icon { font-size: 40px; display: block; margin-bottom: 8px; }
.clf-title { font-family: 'Playfair Display', serif; font-size: 22px; color: #ffffff; font-weight: 700; }
.clf-sub { font-size: 12px; color: rgba(255,255,255,0.55); margin-top: 6px; }
.clf-conf { font-family: 'IBM Plex Mono', monospace; font-size: 20px; font-weight: 700; margin-top: 12px; }
.reg-card { background: linear-gradient(135deg, #1a1200, #2d1f00); border: 2px solid #f0a500; border-radius: 14px; padding: 24px; text-align: center; }
.reg-title { font-size: 11px; color: #f0a500; text-transform: uppercase; letter-spacing: 2px; font-family: 'IBM Plex Mono', monospace; }
.reg-val { font-family: 'Playfair Display', serif; font-size: 36px; font-weight: 900; color: #ffffff; margin: 8px 0; }
.reg-sub { font-size: 12px; color: rgba(255,255,255,0.5); }
.conf-bar-bg { background: #2a2a35; border-radius: 8px; height: 12px; width: 100%; margin-top: 10px; overflow: hidden; }
.conf-bar-fill { height: 100%; border-radius: 8px; }
.stat-pill { background: #1c1c22; border: 1px solid #2a2a35; border-radius: 10px; padding: 14px; text-align: center; }
.pill-val { font-family: 'IBM Plex Mono', monospace; font-size: 18px; font-weight: 600; color: #f0a500; }
.pill-lbl { font-size: 10px; color: #6b6b7a; text-transform: uppercase; letter-spacing: 1.5px; margin-top: 5px; }
div[data-testid="stTabs"] button { font-family: 'IBM Plex Mono', monospace !important; font-size: 11px !important; font-weight: 600 !important; color: #6b6b7a !important; text-transform: uppercase !important; letter-spacing: 2px !important; }
div[data-testid="stTabs"] button[aria-selected="true"] { color: #f0a500 !important; border-bottom-color: #f0a500 !important; }
div[data-testid="stButton"] > button { background: #f0a500 !important; color: #0e0e11 !important; font-family: 'IBM Plex Mono', monospace !important; font-size: 12px !important; font-weight: 700 !important; letter-spacing: 3px !important; text-transform: uppercase !important; border: none !important; border-radius: 8px !important; padding: 14px 32px !important; }
label[data-testid="stWidgetLabel"] p { font-size: 10px !important; color: #6b6b7a !important; text-transform: uppercase !important; letter-spacing: 1.5px !important; }
</style>
...
<div class="masthead">
    ...
    <div class="masthead-stats">
        ...
        <div><div class="mstat-val">Rs.{df['Price_in_Lakhs'].mean():.0f}L</div><div class="mstat-lbl">Avg Price</div></div>
...
kpis = [
    (k1,"🏘️",f"{len(df):,}","Total Properties","up","5.2%"),
    (k2,"💰",f"Rs.{df['Price_in_Lakhs'].mean():.0f}L","Avg Price","up","3.1%"),
    (k3,"✅",f"{df['Good_Investment'].mean()*100:.1f}%","Good Investments","up","2.4%"),
    (k4,"📐",f"{df['Size_in_SqFt'].mean():.0f} ft","Avg Size","down","0.8%"),
    (k5,"🏙️",f"{df['City'].nunique()}","Cities Covered","up","10%"),
    (k6,"⚡",f"{df['Infrastructure_Score'].mean():.1f}/7.7","Avg Infra Score","up","1.5%"),
]
for col,icon,val,lbl,direction,delta in kpis:
    col.markdown(f"""<div class="kpi">
        <span class="kpi-icon">{icon}</span>
        <div class="kpi-num">{val}</div>
        <div class="kpi-lbl">{lbl}</div>
        <div class="kpi-delta {direction}">{"up" if direction=="up" else "down"} {delta} vs last month</div>
...
with tab1:
    ...
    with c1:
        ...
        ax.axvline(fdf['Price_in_Lakhs'].mean(), color=WHITE, linestyle='--', linewidth=1.2,
                   label=f"Mean Rs.{fdf['Price_in_Lakhs'].mean():.0f}L")
        ax.axvline(fdf['Price_in_Lakhs'].median(), color=GREEN, linestyle='--', linewidth=1.2,
                   label=f"Median Rs.{fdf['Price_in_Lakhs'].median():.0f}L")
    ...
    st.markdown('<div class="sec-label">Price Trend Over Time</div>', unsafe_allow_html=True)
    ...
    st.markdown('<div class="panel"><div class="panel-title">Top 15 Cities by Avg Price</div>', unsafe_allow_html=True)
    fig, ax = plt.subplots(figsize=(14, 3.8))
    ...
    ax.bar_label(bars, fmt='Rs.%.0f', padding=3, color=WHITE, fontsize=8.5)
    ...
    if predict_btn:
        ...
        with clf_col:
            if invest == 1:
                st.markdown(f"""<div class="clf-good">
                    ...
                    <div class="clf-sub">Model predicts this property is worth buying</div>
                ...
                st.markdown(f"""<div class="clf-bad">
                    ...
                    <div class="clf-sub">Model predicts this is not a good investment</div>
                    <div class="clf-conf" style="color:#f87171">{conf:.1f}% Confidence</div>
                </div>""", unsafe_allow_html=True)

        with conf_col:
            st.markdown('<div class="panel"><div class="panel-title">Model Confidence Score</div>',
                        unsafe_allow_html=True)
            st.markdown(f"""
            <div style="margin-bottom:16px">
                <div style="display:flex;justify-content:space-between;margin-bottom:6px">
                    <span style="font-size:12px;color:#34d399;font-weight:700">Good Investment</span>
                    <span style="font-family:monospace;color:#34d399;font-weight:700">{conf_good:.1f}%</span>
                </div>
            ...
            <div>
                <div style="display:flex;justify-content:space-between;margin-bottom:6px">
                    ...
                    <span style="font-family:monospace;color:#f87171;font-weight:700">{conf_bad:.1f}%</span>
                </div>
                <div class="conf-bar-bg">
        ...
        with r1:
            st.markdown(f"""<div class="reg-card">
                ...
                <div class="reg-val">Rs.{future_p:.1f}L</div>
                <div class="reg-sub">XGBoost prediction</div>
            </div>""", unsafe_allow_html=True)
        with r2:
            st.markdown(f"""<div class="stat-pill">
                <div class="pill-val" style="color:#34d399">Rs.{profit:.1f}L</div>
        ...
        with r4:
            st.markdown(f"""<div class="stat-pill">
                <div class="pill-val" style="color:#a78bfa">Rs.{ppsf:,.0f}</div>
                <div class="pill-lbl">Price per SqFt</div>
            </div>""", unsafe_allow_html=True)

        # Price projection chart
        st.markdown("<br>", unsafe_allow_html=True)
        st.markdown('<div class="panel"><div class="panel-title">5-Year Price Growth Projection</div>',
                    unsafe_allow_html=True)
        ...
        for y, p in zip(years, prices):
            ax.annotate(f'Rs.{p:.0f}L', (y, p), textcoords="offset points",
        ...
        st.markdown("<br>", unsafe_allow_html=True)
        st.markdown('<div class="sec-label">C · Feature Importance</div>', unsafe_allow_html=True)

        fi1, fi2 = st.columns(2)
        with fi1:
            st.markdown('<div class="panel"><div class="panel-title">Classification - Random Forest Feature Importance</div>',
                        unsafe_allow_html=True)
        ...
        with fi2:
            st.markdown('<div class="panel"><div class="panel-title">Regression - XGBoost Feature Importance</div>',
                        unsafe_allow_html=True)
...
# ════════════════════════════════════════════════════════════
# TAB 3 — CITY INTELLIGENCE & HEATMAPS
# ════════════════════════════════════════════════════════════
with tab3:

    st.markdown('<div class="sec-label">Location-wise Heatmaps</div>', unsafe_allow_html=True)

    c_state = st.selectbox("Filter by State",
        ['All States'] + sorted(df['State'].unique().tolist()), key='city_tab')
    ...
    sns.heatmap(state_bhk, annot=True, fmt='.0f', cmap='YlOrRd',
                ax=ax, linewidths=0.5, linecolor=BG, annot_kws={'size': 9})
    ...
    # Heatmap 2
    st.markdown('<div class="panel"><div class="panel-title">Heatmap 2 · State vs Property Type - Avg Price</div>',
                unsafe_allow_html=True)
    ...
    st.markdown('<div class="panel"><div class="panel-title">Heatmap 4 · City vs Property Type - Avg Price</div>',
                unsafe_allow_html=True)
    fig, ax = plt.subplots(figsize=(14, 5))
    sns.heatmap(heat_df, annot=True, fmt='.0f', cmap='YlOrBr',
                ax=ax, linewidths=0.4, linecolor=BG, annot_kws={'size': 9})
    ax.set_title("Avg Price - City vs Property Type", color=WHITE, fontsize=13, fontweight='bold')
    ...
    st.markdown('<div class="panel"><div class="panel-title">Heatmap 5 · City vs BHK - Infrastructure Score</div>',
                unsafe_allow_html=True)
    fig, ax = plt.subplots(figsize=(14, 5))
    sns.heatmap(infra_heat, annot=True, fmt='.1f', cmap='PuBu',
                ax=ax, linewidths=0.4, linecolor=BG, annot_kws={'size': 10})
    ...
    # Heatmap 6
    st.markdown('<div class="panel"><div class="panel-title">Heatmap 6 · Furnished Status vs Transport - Avg Price</div>',
                unsafe_allow_html=True)
    ...
    sns.heatmap(furn_trans, annot=True, fmt='.0f', cmap='OrRd',
                ax=ax, linewidths=0.5, linecolor=BG, annot_kws={'size': 11})
    ...
    with cc1:
        st.markdown('<div class="panel"><div class="panel-title">Top Cities - Avg Price</div>',
                    unsafe_allow_html=True)
        fig, ax = plt.subplots(figsize=(7, 5))
        ...
        ax.bar_label(bars, fmt='Rs.%.0fL', padding=4, color=WHITE, fontsize=9)
    ...
    deep = [
        (d1,"Avg Price",       f"Rs.{ddf['Price_in_Lakhs'].mean():.1f}L", AMBER),
    ...
    axes[1].axvline(ddf['Price_in_Lakhs'].mean(), color=WHITE, linestyle='--', linewidth=1.5)
    ...
    axes[2].pie(pt_counts.values, labels=pt_counts.index,
    ...
    for ax_ in axes:
        ax_.spines['bottom'].set_color(BORDER)
    fig.patch.set_facecolor(BG)
    fig.tight_layout()
    st.pyplot(fig)
    plt.close()
...
with tab4:
    ...
    st.markdown('<div class="sec-label">Deep Analytics</div>', unsafe_allow_html=True)
    ...
    with da1:
        ...
        bp = ax.boxplot(data_, patch_artist=True,
                        ...
                        flierprops={'marker':'o','markersize':2,
                                    'markerfacecolor':AMBER,'alpha':0.3})
    ...
    with da2:
        st.markdown('<div class="panel"><div class="panel-title">Infrastructure Score vs Price - Good vs Not Good</div>',
                    unsafe_allow_html=True)
        ...
        ax.scatter(bad_['Infrastructure_Score'],  bad_['Price_in_Lakhs'],
                   color=RED,   alpha=0.4, s=8, label='Not Good')
        ax.scatter(good_['Infrastructure_Score'], good_['Price_in_Lakhs'],
                   color=GREEN, alpha=0.5, s=8, label='Good Investment')
